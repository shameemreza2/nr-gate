# 第一阶段 — 声调基础 + HSK 3 词汇冲刺

**在抵达前建立 HSK 3 词汇与识字基础。**
Day 0 = **2026年7月3日（周五）** · 结束日/出发日 = **2026年8月22日（周六）** · **48张核心日卡**（Day 0 → Day 47）+ **2天缓冲**（Day 48–49）。

本文件夹为*引擎*。每张 `day-NN.md` 是一份含**一个强制门控**的执行清单。未通过该日门控则不推进。git 提交历史（每通过一天提交一次）即为执行小时数日志——父级可行性报告所列的唯一关键预测变量。

---

## §0 · 单元制 Unit system — how any day-card actually runs (added 2026-07-09)

Day cards are unchanged; this section governs **how you execute them**. `day-00.md` is the worked example.

- **Unit** = a 15–25 min timeboxed slice with one ✔ win condition. For every card: one block ≈ 1–2 units — split a 45–75 min block at its natural checkpoint (e.g. "SRS +10 words" → "SRS first 5" + "SRS last 5 + worst-3 re-record").
- **MIN DAY = 1 unit (≥20 min), tone or SRS reps preferred.** This is §3's own law — "SRS floor = 20 min/day, never zero" — made concrete. One unit keeps the streak alive. It does **not** pass the card; it keeps you in the game, which is the thing that actually predicts landing state.
- **FULL DAY** = enough units to attempt the hard gate → `dayNN:` PASS commit (hook rules unchanged).
- **HOT DAY** = chain past the gate into the next card's units. Two *cards* in one day: allowed **only in Stage B (Day 22+)** — tone acquisition is day-gated, not hour-gated (§2). Stage A never doubles; hot Stage-A energy goes into extra HVPT reps or tomorrow's SRS block instead.
- **Commits are the streak — no new tooling.** After any unit: `unit: dayNN <what> | <min>` (sails through the hook). On gate pass: `dayNN: ...` as before (§8). Streak = consecutive calendar days with ≥1 commit of either kind; `git log --oneline` is the whole scoreboard.
- **Session opener ritual (90 s):** `uv run python scripts/tone_compare.py --record ma2` — any syllable. A verdict and a picture before any decision about how much to do. Start = play.
- **Artifact ritual (information-gap engine, added 2026-07-09).** Day N has a matching real-world text in `phase1/artifacts.md` (pinyin place-names → signs → menus → notices → WeChat, graded to the vocab curve). After the opener: 60 seconds, attempt artifact N aloud, note exactly what you can't read — that itch is the day's fuel, don't scratch it yet. The day's LAST unit: re-attempt it, then Pleco-check. The before/after delta is the visible score. Never pre-study an artifact; the gap is the mechanism (Loewenstein: a felt knowledge gap drives like hunger).
- **Guess before every reveal.** HVPT: say the tone BEFORE the answer plays. SRS: answer aloud BEFORE flipping. A wrong committed guess beats a right passive nod — prediction-with-feedback is what encodes, exposure isn't (productive-failure evidence: attempt-first ≈ 2–3× transfer).
- **A broken streak costs the streak, nothing else.** Slips already have their price (§7 auto-shrink — scope, not shame). Log it, forgive it — self-forgiveness after a lapse measurably reduces the next lapse (Wohl et al. 2010) — then take the next MIN DAY.

---

### 官方 HSK 字数 — 直接从大纲核实

来源：**`新版HSK考试大纲1219.pdf`** = *中文水平考试 HSK 考试大纲*，中外语言交流合作中心，**2025年11月出版，2026年7月起生效**。直接从词汇大纲/汉字大纲列表解析（连续序号，等级标注）。**[R]**

| 等级 | 累计词汇量 | 累计认读字 | 书写字 |
|---|---|---|---|
| HSK 1 | **300** | 246 | 无 |
| HSK 2 | **500** | 371 | 无 |
| **HSK 3** | **1,000** | **655** | +150（书写起始） |
| HSK 4 | 2,000 | 1,096 | +150 |
| HSK 5 | 3,600 | 1,527 | +150 |
| HSK 6 | 5,400 | 1,940 | +150 |
| HSK 7–9 | 11,000 | 3,088 | +500 |

> 修正了早期草稿中"HSK 2 ≈ 1,272词"的错误（那是2021年标准GF0025-2021，**非**本考试大纲）。以上经核实的数字与父级报告一致。

---

## 1. 固定参数

| 参数 | 值 |
|---|---|
| Day 0 | 2026年7月3日（周五） |
| 结束日/出发日 | 2026年8月22日（周六）→ 第一阶段 = **48天核心课程 + 2天缓冲 = 50天** |
| 每日时长 | 目标 **6小时**，上限6.5小时，下限4小时 → **总计约250–280小时** |
| **声调硬目标（结束日）** | **声调对识别率 ≥ 97%**，T2↔T3 ≥ 95% |
| **词汇扩展解锁条件** | 声调对识别率 **≥ 95%** + T2↔T3 ≥ 92%（目标约 Day 21，受门控约束） |
| **词汇目标（结束日）** | **约1,075词 = 完整 HSK 3 词汇（1,000）+ 约75个 HSK 4 词**；下限约850（HSK 3 ~85%） |
| 汉字范围 | **认读 + 输入法输入 → 约655认读字（完整 HSK 3 识字集）同步掌握。**书写（书写字）推迟至第二阶段。 |

**第一阶段实际交付内容：** 结束时掌握 **HSK 3 全部词汇和认读字集**，声调达 ≥97%。这使 HSK 3 目标中最难的部分在抵达前完成。第二阶段只需新增：**考试技巧、150个书写字、口语（HSKK 初级）**——比从零开始可信得多。

---

## 2. 数字依据（可行性结论简版）

- **高变量声调训练（Wang et al. 1999, JASA）：** 非声调母语成人约8次训练/约2周后，声调识别率从约69%提升至约90%，并可保持与泛化。**[R]**
- **90% → 97% 是漫长的、依赖资质的尾段**，T2 是已记录的最难声调类别，97% 不是48天内的保证结果。**[E ← R]**
- **声调受间隔限制，不可突击。** 全天时间不等于声调更快——它受日控，不受小时控。**[D ← R]**
- **持久词汇摄入约 20–40 新词/天**；超过约60词/天即为应试记忆，遗忘快速。**[E/D]**

**结论 "声调优先至97%再冲刺"**：作为*词汇阻断器*被否决——97%门控若始终未通过则可能耗尽整个窗口。保护化石化在约90–95%感知+产出规律时到位，不需要97%。

**最终决定：** 在 **95%解锁**（更安全的声调，约Day 21）。预计着陆状态：**约1,075词（完整 HSK 3 词汇）+ ≥95–97% 声调**。

---

## 3. 常规定律（永不违反）

1. **声调门控至上。** 当日声调识别率目标为强制门控。未达标 → 明天重复同一门控，不添加新内容（**滑坡规则**，§7）。声调不等词汇；词汇等声调。
2. **SRS下限 = 20分钟/天，永不为零。** 减量日、旅途日均不例外。
3. **95%解锁前不进行词汇扩展。** A阶段允许保持小量 **HSK 1 词库（≤300词，全部经声调核查）**；扩展 HSK 2 → HSK 3 仅在解锁后进行。
4. **产出规律（防化石化）。** 每张新词卡至少一次录音并与母语人士音频对比。每周周日声调审计。
5. **认读 + 输入法输入。** 第一阶段不进行手写（HSK 1–2无书写要求；HSK 3 的150书写字为第二阶段任务）。
6. **重点纠正项：** 强化 T2↔T3 最小对比练习；正音 zh/ch/sh/r vs j/q/x vs z/c/s；分离 ü（lǜ / nǚ）。
7. **每通过一天一次提交。** 提交信息即为日志（§8）。

---

## 4. 两阶段结构

| 阶段 | 天数 | 声调 | 词汇 | 目标 |
|---|---|---|---|---|
| **A — 声调建立** | Day 0 – ~21 | 全力：拼音 → 单声调 → 声调对 → **≥95%** | **HSK 1 词库，≤300词**，声调核查 | 通过解锁门控 |
| **解锁** | ~Day 21 | 对识别率 ≥95% + T2↔T3 ≥92% | — | 开放词汇扩展 |
| **B — 词汇冲刺** | ~Day 22 – 47 | 维持 + 推至 **≥97%**；开始连续语音产出 | 扩展 **HSK 2 → HSK 3（500 → 1,000）**，溢出至 HSK 4 | 完整 HSK 3 词汇 |

解锁日受门控约束：日历显示约 Day 21，若声调滞后则解锁右移，词汇目标自动缩减（§7）。

---

## 5. 声调门控阶梯（主干）

指标 = 全新 ≥40 题、多说话人测试的**声调对识别率**（第1周为单声调4选1测试）。从 HVPT 语料库构建测试；不复用已训练素材。

| 结束周 | 天数 | 整体门控 | T2↔T3 门控 | 本周新增 |
|---|---|---|---|---|
| 第1周 | 6 | 单声调识别 **≥85%** | — | 全拼音解码；ü 稳固；4个声调产出 |
| 第2周 | 13 | 声调对识别 **≥85%** | ≥78% | 声调对 HVPT；T3 变调入门 |
| 第3周 | 20 | 声调对识别 **≥92%** | ≥88% | 多说话人；双音节词声调 |
| **解锁** | **21** | **声调对识别 ≥95%** | **≥92%** | **→ 词汇扩展开放** |
| 第4–6周 | 22–41 | 推进 **96 → 97%** | 93 → 95% | 连续语音产出声调 |
| 第7周 | 42–47 | 维持 **≥97%** | ≥95% | 速度下考试级声调稳定性 |

---

## 6. 词汇/汉字增长曲线

A阶段（HSK 1 词库）缓慢积累，每词经声调核查。B阶段从大纲 PDF 中的官方 HSK 2 和 HSK 3 词汇表扩展。每日累计目标见**计划图（§10）**。

轨迹：约75（第1周）→ 约195（第2周）→ **300 = HSK 1 完成（Day 20）** → **约530 = HSK 2 完成（约Day 29）** → **1,000 = HSK 3 词汇完成（约Day 44）** → **约1,075（结束日）**（溢出约75个 HSK 4 词）。认读字同步达 **约655（完整 HSK 3 识字集）**。

---

## 7. 门控/推进/滑坡机制（"达标后才推进"）

- **计划日 ≠ 日历日。** 日卡仅在强制门控通过时推进。
- **通过：** 记录，提交，继续。**滑坡（门控未达标）：** 次日重复同一门控区块，不添加新内容。记录为 `dayNN-r1`、`-r2`……
- **声调滑坡是主要风险**（阶梯陡峭）。预算约吸收 3–4 次。
- **自动缩减：** 每次滑坡将解锁日和结束日词汇目标下调约35词。在 `tracker.md` 中记录运行中的调整目标。结束日不移动——第二阶段继承剩余内容。

---

## 8. Git / 提交规范

每天门控通过时提交一次：

```
git -C "D:\hsk3~5" add phase1/ tracker.md
git -C "D:\hsk3~5" commit -m "dayNN: <门控结果> | <小时> | vocab <累计>"
# 例：day13: pair-ID 86% PASS | 6.0h | vocab 195
# 滑坡：day13-r1: pair-ID 81%（目标85）| 5.5h | vocab 195 HOLD
```

`git -C "D:\hsk3~5" log --oneline` 即为小时数+门控历史记录——唯一真实预测指标。

---

## 9. Day-0 准备（计为学习时间）

安装 **Pleco**（付费插件值得）+ **Anki**。从大纲 PDF 词汇表构建/导入**官方 HSK 1 → HSK 3 词卡**（HSK 1 = 300，HSK 2 = +200，HSK 3 = +500）。设置 **HVPT 声调感知素材**（高变量、多说话人）。标记 PDF + chinesetest.cn。固定每日时间块。**录制朗读10个拼音音节**——基线录音。

---

## 10. 完整50天计划图（48天核心课程 + 2天缓冲）

S = 阶段（A / U=解锁 / B）。词汇 = 累计新词目标。减量日 = 每周一次（模拟+复习，半量，不引入新内容，不再固定落在周日——见下方实际星期）。★ = 跨越 HSK 等级边界。Day 48–49 为出发前缓冲（原48天核心课程末尾多出的2天余量，用于补做滑坡门控或纯巩固+打包）。

| Day | 日期 | 周 | S | 声调强制门控 | 词汇 | 重点 |
|---|---|---|---|---|---|---|
| 0 | 周五 7月3 | 1 | A | 设置完成 + 5个音节上4个声调产出 | 0 | §9 准备；拼音总览；4个声调 |
| 1 | 周六 7月4 | 1 | A | 单声调识别 ≥70% | 10 | 声母 b p m f d t n l |
| 2 | 周日 7月5 | 1 | A | 单声调识别 ≥75% | 25 | g k h, j q x |
| 3 | 周一 7月6 | 1 | A | 识别 ≥78% + 翘舌/齿音对 ≥70% | 40 | zh ch sh r, z c s |
| 4 | 周二 7月7 | 1 | A | 拼音听写20个音节 ≥80% | 40 | 减量：第1周复习+模拟 |
| 5 | 周三 7月8 | 1 | A | 识别 ≥80% + ü 产出 | 55 | 韵母；ü 分离（lǜ/nǚ） |
| 6 | 周四 7月9 | 1 | A | **单声调识别 ≥85% + 拼音完成** | 75 | 复合韵母 |
| 7 | 周五 7月10 | 2 | A | 声调对识别 ≥70% | 95 | 声调对入门 |
| 8 | 周六 7月11 | 2 | A | 声调对识别 ≥74% + T2↔T3 ≥60% | 115 | T2 vs T3 专攻 |
| 9 | 周日 7月12 | 2 | A | 声调对识别 ≥77% + T2↔T3 ≥65% | 135 | T2 vs T1 专攻 |
| 10 | 周一 7月13 | 2 | A | 声调对识别 ≥80% + T2↔T3 ≥68% | 155 | 多说话人声调对 |
| 11 | 周二 7月14 | 2 | A | 声调对识别 ≥80% 稳定 | 155 | 减量：模拟+复习 |
| 12 | 周三 7月15 | 2 | A | 声调对识别 ≥83% + T2↔T3 ≥72% | 175 | T3 变调（3-3→2-3） |
| 13 | 周四 7月16 | 2 | A | **声调对识别 ≥85% + T2↔T3 ≥78%** | 195 | 第2周门控 |
| 14 | 周五 7月17 | 3 | A | 声调对识别 ≥86% + T2↔T3 ≥80% | 215 | 新说话人 |
| 15 | 周六 7月18 | 3 | A | 声调对识别 ≥88% + T2↔T3 ≥82% | 235 | 双音节词声调 |
| 16 | 周日 7月19 | 3 | A | 声调对识别 ≥89% + T2↔T3 ≥84% | 255 | 双音节+速度 |
| 17 | 周一 7月20 | 3 | A | 声调对识别 ≥90% + T2↔T3 ≥86% | 275 | 混合说话人 |
| 18 | 周二 7月21 | 3 | A | 声调对识别 ≥90% 稳定 | 275 | 减量：模拟+复习 |
| 19 | 周三 7月22 | 3 | A | 声调对识别 ≥91% + T2↔T3 ≥87% | 290 | 解锁前冲刺 |
| 20 | 周四 7月23 | 3 | A | **声调对识别 ≥92% + T2↔T3 ≥88%** | 300 ★ | 第3周门控；**HSK 1 词汇完成** |
| 21 | 周五 7月24 | 4 | **U** | **解锁：声调对识别 ≥95% + T2↔T3 ≥92%** | 300 | 解锁；构建 HSK 2→3 扩展词卡 |
| 22 | 周六 7月25 | 4 | B | 声调 ≥95% 维持 + 30新词保留 ≥85% | 330 | 冲刺开始；语法 是/有/在 |
| 23 | 周日 7月26 | 4 | B | 声调 ≥95% + 30新词 | 360 | 量词 个/位/张 |
| 24 | 周一 7月27 | 4 | B | 声调 ≥95% + 30新词 | 390 | 的；疑问词 吗/什么 |
| 25 | 周二 7月28 | 4 | B | 减量模拟：声调对识别 ≥95% 稳定 | 390 | 减量：声调审计+复习 |
| 26 | 周三 7月29 | 4 | B | 声调 ≥95% + 35新词 | 425 | 了（体貌/变化） |
| 27 | 周四 7月30 | 4 | B | 声调 ≥96% + 35新词 | 460 | 想/要/会/能 |
| 28 | 周五 7月31 | 5 | B | 声调 ≥96% + 35新词 | 495 | 时间+日期 |
| 29 | 周六 8月1 | 5 | B | 声调 ≥96% + 35新词 | 530 ★ | 位置；**HSK 2 词汇完成** |
| 30 | 周日 8月2 | 5 | B | 声调 ≥96% + 35新词 | 565 | 在…呢 / 进行体 |
| 31 | 周一 8月3 | 5 | B | 声调 ≥96% + 35新词 | 600 | 比较 比 |
| 32 | 周二 8月4 | 5 | B | 减量模拟：声调对识别 ≥96% | 600 | 减量：声调审计+复习 |
| 33 | 周三 8月5 | 5 | B | 声调 ≥97% 目标 + 35新词 | 635 | 过（经历） |
| 34 | 周四 8月6 | 5 | B | 声调 ≥97% + 35新词 | 670 | 结果补语入门 |
| 35 | 周五 8月7 | 6 | B | 声调 ≥97% + 40新词 | 710 | 把 入门（认读） |
| 36 | 周六 8月8 | 6 | B | 声调 ≥97% + 40新词 | 750 | 连续语音产出 |
| 37 | 周日 8月9 | 6 | B | 声调 ≥97% + 40新词 | 790 | 自我介绍草稿（第二阶段种子） |
| 38 | 周一 8月10 | 6 | B | 声调 ≥97% + 40新词 | 830 | HSK 3 速度听力 |
| 39 | 周二 8月11 | 6 | B | 减量模拟：完整 HSK 2 级听力 | 830 | 减量：声调审计+复习 |
| 40 | 周三 8月12 | 6 | B | 声调 ≥97% + T2↔T3 ≥95% + 40新词 | 870 | 弱项汉字复习 |
| 41 | 周四 8月13 | 6 | B | 声调 ≥97% + 35新词 | 910 | 第6周巩固 |
| 42 | 周五 8月14 | 7 | B | 声调 ≥97% + 35新词 + HSK 3 模拟第1节 | 945 | HSK 3 听力+阅读模拟 |
| 43 | 周六 8月15 | 7 | B | 声调 ≥97% + 35新词 + 错误清除 | 980 | HSK 3 模拟第2节 |
| 44 | 周日 8月16 | 7 | B | 声调 ≥97% + 35新词 | 1015 ★ | **HSK 3 词汇完成**；弱项钻研 |
| 45 | 周一 8月17 | 7 | B | 声调 ≥97% + 30新词 + 完整 HSK 3 模拟 | 1045 | 完整 HSK 3 听力+阅读模拟 |
| 46 | 周二 8月18 | 7 | B | 减量：声调对识别 ≥97% + 自我介绍流利 | 1045 | 减量：声调审计+整理行李 |
| 47 | 周三 8月19 | 7 | B | **核心课程出口门控：声调 ≥97%，词汇 ≥1,000（HSK 3），自我介绍60秒** | 1075 | 最终复习；行程安排 |
| 48 | 周四 8月20 | 8 | B | 声调 ≥97% 维持（缓冲：如Day47未过则补做） | 1075 | 缓冲日1：SRS+打包 |
| 49 | 周五 8月21 | 8 | B | **出发就绪：声调 ≥97%，自我介绍流利** | 1075 | 缓冲日2：最终检查+打包完毕 |
| — | 周六 8月22 | — | — | **✈ 出发** | — | 第二阶段在国内开始 |

---

## 11. 每日模板（约6小时）

| 模块 | A阶段 | B阶段 |
|---|---|---|
| 声调感知（HVPT） | 0:45 | 0:30（维持） |
| 产出：录音与母语对比，记录最差错误 | 0:45 | 0:30 |
| 拼音（A阶段）/ 语法模式（B阶段） | 0:30 | 0:40 |
| **SRS词汇+汉字（新词+复习）** | 1:15 | **2:30**（主要模块） |
| 听力（分级） | 0:45 | 0:45 |
| 阅读（拼音→汉字） | 0:30 | 0:35 |
| 记录+git提交 | 0:20 | 0:20 |
| **≈ 合计** | **~5:30** | **~6:10** |

周日 = 减量：SRS下限+一次模拟+每周声调审计（约3小时）。

---

## 12. 第二阶段覆盖内容（另行说明）

在国内推进 **HSK 3 笔试 + HSKK 初级（口语）+ 专业词卡 + 中文摘要阅读**。第一阶段已完成 HSK 3 的**词汇（1,000）+ 认读字（655）**，第二阶段仅需新增：**(a)** HSK 3 考试技巧 + **150书写字**，**(b)** HSKK 口语流利度，**(c)** 专业词卡+摘要阅读。第二阶段继承第一阶段剩余的词汇/声调内容。
