...
chmod +x hooks/commit-msg hooks/gate-ci.sh 2>/dev/null || true
...
echo "Server-side lock: the same check runs in GitHub Actions on every push to main."
echo "The required status check 'gate' must pass before the push is accepted."
